...
export default function Tables() {
  ...
  const fetchFarmerProducts = async () => {
    try {
      ...
      const unsoldProducts = response.data.filter(product => product.farmerStatus === 'unsold');
  ...
  const createOrder = async (productId, productName, userID) => {
    try {
      const response = await http.post('/orders', {
        buyerIds: [userData._id],
        ownerId: userID,
        productId: productId,
      ...
      // Update the farmerStatus of the product to "sold"
      await http.put(`/products/${productId}`, { farmerStatus: 'sold' });

      // Fetch updated farmer products after successful order creation
      fetchFarmerProducts();

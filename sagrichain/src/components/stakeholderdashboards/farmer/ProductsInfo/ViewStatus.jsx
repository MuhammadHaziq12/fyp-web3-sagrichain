...
export default function Tables() {
  ...
  const fetchOrders = async (userId) => {
    try {
      ...
      // Filter orders where buyerIds[0] is not null
      const userOrders = response.data.filter(order => order.buyerIds[0] !== null && order.ownerId === userId);
                  ...
                  <tbody>
                    {orders.map((order) => (
                      <tr key={order._id}>
                        ...
                        <td className="border px-4 py-2 whitespace-nowrap">
                          {order.buyerIds[0] !== null ? order.buyerIds[0] : 'N/A'}
                        </td>
                      </tr>
                    ))}
                  </tbody>

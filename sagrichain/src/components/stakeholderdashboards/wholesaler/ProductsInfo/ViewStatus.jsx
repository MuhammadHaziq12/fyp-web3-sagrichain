...
export default function Tables() {
  ...
  const fetchOrders1 = async (userId) => {
    try {
      ...
      const userOrders1 = ordersData.filter(order1 => {
        ...
        return order1.buyerIds[1] === userId && product && product.wholesalerStatus === "sold";
      });
  ...
  const fetchOrders2 = async (userId) => {
    try {
      const response = await http.get("/orders");
      const userOrders2 = response.data.filter(order2 => order2.buyerIds[2]);
                  ...
                  <tbody>
                    {orders1.map((order1) => (
                      <tr key={order1._id}>
                        ...
                        <td className="border px-4 py-2 whitespace-nowrap">
                          {orders2.find(order2 => order2.productId === order1.productId)?.buyerIds[2]}
                        </td>
                      </tr>
                    ))}
